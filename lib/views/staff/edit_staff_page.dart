...
class EditStaffPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    String? hintText;
    if (isEditingOwner) {
      hintText = 'The owner\'s role cannot be changed.';
    } else if (isManagerEditingAdmin) {
      hintText = 'Only an owner can change an admin\'s role.';
    }

    return Scaffold(
      appBar: AppBar(title: Text('Edit Staff: ${staff.displayName}')),
      body: Padding(
          ...
          children: [
            ...
            DropdownButtonFormField<UserRole>(
              ...
              decoration: InputDecoration(
                ...
                fillColor: isFormDisabled ? Colors.grey.withOpacity(0.2) : null,
              ),
            ),
            if (hintText != null)
              Padding(
            ...
            if (!isEditingOwner) // An owner cannot block themselves
              ElevatedButton(
                ...
                child: Text(staff.isDisabled ? 'Unblock User' : 'Block User'),
              ),
          ],
        ),
      ),
